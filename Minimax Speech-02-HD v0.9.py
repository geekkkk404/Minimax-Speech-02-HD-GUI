...
class LanguageManager:
    def __init__(self, language_folder="langs", default_lang="en_US"):
        ...
        self.language_folder = os.path.join(script_dir, language_folder)
...
class SpeechApp(ctk.CTk):
        ...
        for config in self.PARAMETER_CONFIG:
            ...
            if control_type == "slider":
                ...
                if param_id == "voice_id":
                    ...
                    # --- NEW: Create a dedicated label for the custom entry ---
                    self.custom_voice_id_label = ctk.CTkLabel(parent_frame, text="")
                    self.custom_voice_id_label.grid(row=row_counter + 1, column=0, padx=10, pady=5, sticky="w")
                    self.custom_voice_id_label.grid_remove() 
                    ...
                    self.param_widgets['custom_voice_id_label'] = self.custom_voice_id_label
                    self.param_widgets['custom_voice_id_entry'] = self.custom_voice_id_entry
    ...
    def on_voice_id_change(self, choice):
        """当 voice_id 下拉框选项改变时调用"""
        lm = self.lang_manager
        is_custom = (choice == lm.get("voice_custom_option"))

        if is_custom:
            # --- UPDATED: Show both the label and the entry ---
            self.custom_voice_id_label.grid()
            self.custom_voice_id_entry.grid()
            self.custom_voice_id_entry.configure(state="normal")
            self.custom_voice_id_entry.focus()
        else:
            # --- UPDATED: Hide both the label and the entry ---
            self.custom_voice_id_label.grid_remove()
            self.custom_voice_id_entry.grid_remove()
    ...
    def update_ui_language(self):
        ...
        for config in self.PARAMETER_CONFIG:
            param_id = config["id"]
            widgets = self.param_widgets.get(param_id)
            ...
            if "json_map" in config:
                ...
                if param_id == "voice_id":
                    # --- UPDATED: Use a clearer option text from JSON ---
                ...
                if options_display:
                    ...
                    if current_value not in options_display and param_id == "voice_id" and self.custom_voice_id_var.get() == "":
                        self.param_vars[param_id].set(options_display[0])
                    elif current_value not in options_display:
                         self.param_vars[param_id].set(options_display[0])

        # --- NEW: Update the new custom widgets with translated text ---
        self.param_widgets['custom_voice_id_label'].configure(text=lm.get("param_voice_id_custom"))
        self.param_widgets['custom_voice_id_entry'].configure(placeholder_text=lm.get("custom_voice_id_placeholder"))
    ...
    def set_ui_state(self, is_generating):
        ...
        for param_id, widgets in self.param_widgets.items():
            # Special handling for custom entry which is not in PARAMETER_CONFIG
            if param_id in ['custom_voice_id_label', 'custom_voice_id_entry']:
                continue
            if 'control' in widgets: 
                widgets['control'].configure(state=state)
        
        # Explicitly handle the state of the custom voice entry
        if self.param_vars["voice_id"].get() == self.lang_manager.get("voice_custom_option"):
    ...
    def run_generation(self):
        ...
        try:
                ...
                if config["type"] == "checkbox":
                    ...
                    if param_id == "voice_id":
                        if raw_value == lm.get("voice_custom_option"):
                            ...
                            if not custom_id:
                                # --- UPDATED: Use new, clearer error message ---
            ...
            temp_dir = os.path.join(os.path.dirname(sys.argv[0]) or '.', "temp_audio")
